...
const LEDDetector: React.FC = () => {
  ...
  const processFrame = () => {
    ...
    for (let i = 0; i < data.length; i += 16) {
      // Sample every 4th pixel (4 channels per pixel)
    ...
    const movingAvg = brightnessHistoryRef.current.reduce((a, b) => a + b) / brightnessHistoryRef.current.length;
    ...
    if (Math.abs(currentSlope) > threshold) {
      ...
      if (lastStateRef.current === "rising" && newState === "falling") {
        ...
        if (peakTimesRef.current.length >= 4) {
          ...
          const matchedFreq = expectedFreqs.reduce((prev, curr) => (Math.abs(curr - freq) < Math.abs(prev - freq) ? curr : prev));
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="p-4 bg-gray-100 rounded-lg space-y-2">
            <div className="text-xl font-bold">{stats.frequency ? `${stats.frequency} Hz` : "No frequency detected"}</div>
            <div>Peaks: {stats.peaks}</div>
            <div>Brightness: {Math.round(stats.currentBrightness)}</div>
            <div>
              Range: {Math.round(stats.minBrightness)} - {Math.round(stats.maxBrightness)}
            </div>
            <div className="w-full bg-gray-200 h-2 rounded-full overflow-hidden">
              <div
                className="h-full bg-yellow-500 transition-all duration-100"
                style={{
                  width: `${((stats.currentBrightness - stats.minBrightness) / (stats.maxBrightness - stats.minBrightness)) * 100}%`,
                }}
              />
            </div>
          </div>
